...
|:---|:---|:---:|:---|
| `-GetCache` | SwitchParameter | ☐ | Return the in-memory cache hashtable |
| `-Language` | String | ✅ | BCP 47 language code for the cache entry |
| `-Key` | String | ✅ | The source text to cache |
| `-Value` | String | ✅ | The translated text to store |
| `-PersistNow` | SwitchParameter | ✅ | Write all dirty caches to disk and clear<br>memory |
| `-ClearCache` | SwitchParameter | ✅ | Clear the in-memory cache and delete all disk<br>files |
| `-PurgeFromCache` | SwitchParameter | ✅ | Remove untranslated entries from all language<br>caches on disk |
