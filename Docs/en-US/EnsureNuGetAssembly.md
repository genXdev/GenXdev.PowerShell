...
|:---|:---|:---:|:---|
| `-PackageKey` | String | ✅ | Package key from packages.json or direct<br>NuGet PackageId. |
| `-ManifestPath` | String | ☐ | Path to packages.json; defaults to module<br>root if omitted. |
| `-Version` | String | ☐ | Specific version; if omitted, use highest<br>from JSON or latest. |
| `-TypeName` | String | ☐ | TypeName to verify loading. |
| `-ForceLatest` | SwitchParameter | ☐ | Fallback to latest if exact version fails. |
| `-Destination` | String | ☐ | Custom install destination; defaults to local<br>persistent or global cache. |
| `-Description` | String | ☐ | Optional description of the software and its<br>purpose for consent. |
| `-Publisher` | String | ☐ | Optional publisher or vendor of the software<br>for consent. |
| `-AutoConsent` | SwitchParameter | ☐ | Force a prompt even if preference is set for<br>consent. |
