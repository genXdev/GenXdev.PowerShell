...
|:---|:---|:---:|:---|
| `-Query` | String | ✅ | The query string for analyzing the image |
| `-ImagePath` | String | ✅ | Path to the image file for analysis |
...
| `-AudioTemperature` | Double | ☐ | Temperature for audio response generation. |
| `-TemperatureResponse` | Double | ☐ | Temperature for the response generation. |
| `-Language` | String | ☐ | The language for generated descriptions and <br>keywords |
| `-CpuThreads` | Int32 | ☐ | Number of CPU threads to use. |
| `-SuppressRegex` | String | ☐ | Regular expression to suppress output. |
...
| `-DontSpeak` | SwitchParameter | ☐ | If specified, do not speak the output. |
| `-DontSpeakThoughts` | SwitchParameter | ☐ | If specified, do not speak the model's<br>thoughts. |
| `-NoVOX` | SwitchParameter | ☐ | If specified, disables VOX. |
| `-UseDesktopAudioCapture` | SwitchParameter | ☐ | If specified, use desktop audio capture. |
| `-NoContext` | SwitchParameter | ☐ | If specified, disables context usage. |
| `-WithBeamSearchSamplingStrategy` | SwitchParameter | ☐ | If specified, use beam search sampling<br>strategy. |
| `-OnlyResponses` | SwitchParameter | ☐ | If specified, only return responses. |
| `-DontAddThoughtsToHistory` | SwitchParameter | ☐ | If specified, don't add thoughts to the<br>history. |
| `-ContinueLast` | SwitchParameter | ☐ | Continue the last conversation. |
| `-Speak` | SwitchParameter | ☐ | Speak the output. |
| `-SpeakThoughts` | SwitchParameter | ☐ | Speak the model's thoughts. |
| `-OutputMarkdownBlocksOnly` | SwitchParameter | ☐ | Output only markup blocks. |
| `-MarkupBlocksTypeFilter` | String[] | ☐ | Filter for markup block types. |
| `-ChatOnce` | SwitchParameter | ☐ | If specified, only chat once. |
| `-NoSessionCaching` | SwitchParameter | ☐ | Disable session caching. |
