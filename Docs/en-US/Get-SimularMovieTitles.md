...
|:---|:---|:---:|:---|
| `-Movies` | String[] | ✅ | Array of movie titles to analyze for<br>similarities |
...
| `-OpenInImdb` | SwitchParameter | ☐ | Opens IMDB searches for each result |
| `-Language` | String | ☐ | Language for IMDB search or browser session |
| `-Monitor` | Int32 | ☐ | Monitor index or name for browser window<br>placement |
| `-Width` | Int32 | ☐ | Width of the browser window in pixels |
| `-Height` | Int32 | ☐ | Height of the browser window in pixels |
| `-AcceptLang` | String | ☐ | Accept-Language HTTP header for browser<br>session |
| `-Private` | SwitchParameter | ☐ | Open browser in private/incognito mode |
| `-Chrome` | SwitchParameter | ☐ | Use Google Chrome for browser session |
| `-Chromium` | SwitchParameter | ☐ | Use Chromium for browser session |
| `-Firefox` | SwitchParameter | ☐ | Use Mozilla Firefox for browser session |
| `-Left` | Int32 | ☐ | Left position of the browser window in pixels |
| `-Right` | Int32 | ☐ | Right position of the browser window in<br>pixels |
| `-Bottom` | Int32 | ☐ | Bottom position of the browser window in<br>pixels |
| `-Centered` | SwitchParameter | ☐ | Open browser window centered on screen |
| `-FullScreen` | SwitchParameter | ☐ | Open browser in full screen mode |
| `-ApplicationMode` | SwitchParameter | ☐ | Open browser in application mode (minimal UI) |
| `-NoBrowserExtensions` | SwitchParameter | ☐ | Disable browser extensions for session |
| `-DisablePopupBlocker` | SwitchParameter | ☐ | Disable popup blocker in browser session |
| `-FocusWindow` | SwitchParameter | ☐ | Focus the browser window after opening |
| `-SetForeground` | SwitchParameter | ☐ | Set browser window to foreground after<br>opening |
| `-Maximize` | SwitchParameter | ☐ | Maximize the browser window after opening |
| `-RestoreFocus` | SwitchParameter | ☐ | Restore focus to previous window after<br>closing browser |
| `-NewWindow` | SwitchParameter | ☐ | Open each IMDB result in a new browser window |
| `-ReturnURL` | SwitchParameter | ☐ | Return the URL after opening IMDB search |
| `-ReturnOnlyURL` | SwitchParameter | ☐ | Return only the URL without opening browser |
| `-SendKeyEscape` | SwitchParameter | ☐ | Send Escape key to browser after opening |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | ☐ | Hold keyboard focus in browser after sending<br>keys |
| `-SendKeyUseShiftEnter` | SwitchParameter | ☐ | Use Shift+Enter when sending keys to browser |
| `-SendKeyDelayMilliSeconds` | Int32 | ☐ | Delay in milliseconds between sending keys to<br>browser |
| `-NoBorders` | SwitchParameter | ☐ | Open browser window without borders |
...
| `-SideBySide` | SwitchParameter | ☐ | Open browser windows side by side for each<br>result |
...
| `-ImageDetail` | String | ☐ | Image detail level for image processing. |
| `-Functions` | Collections.Hashtable[] | ☐ | Array of function definitions that can be<br>called by the AI model during processing. |
| `-ExposedCmdLets` | GenXdev.Helpers.ExposedCmdletDefinition[] | ☐ | Array of PowerShell command definitions to<br>use as tools that the AI can invoke. |
| `-NoConfirmationToolFunctionNames` | String[] | ☐ | Array of command names that don't require<br>confirmation before execution. |
| `-AudioTemperature` | Double | ☐ | Temperature for audio generation. |
| `-TemperatureResponse` | Double | ☐ | Temperature for response generation. |
...
| `-SilenceThreshold` | Double | ☐ | Silence threshold for audio processing. |
...
| `-DontSpeakThoughts` | SwitchParameter | ☐ | Disable thought speech output. |
...
| `-SetClipboard` | SwitchParameter | ☐ | When specified, copies the resulting string<br>list back to the system clipboard after<br>processing. |
