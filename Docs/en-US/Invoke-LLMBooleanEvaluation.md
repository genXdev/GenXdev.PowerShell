...
> **SubModule:** GenXdev.AI.Queries | **Type:** Function | **Aliases:** `equalstrue`
...
| `-Instructions` | String | ☐ | Instructions for the AI model on how to<br>evaluate  the statement |
...
| `-DontAddThoughtsToHistory` | SwitchParameter | ☐ | Don't add model thoughts to conversation<br>history |
...
| `-AllowDefaultTools` | SwitchParameter | ☐ | Allow AI to use default tools and<br>capabilities |
...
| `-Language` | String | ☐ | Language code or name for the response |
| `-CpuThreads` | Int32 | ☐ | Number of CPU threads to use for processing |
| `-SuppressRegex` | String | ☐ | Regular expression to suppress from output |
...
| `-NoContext` | SwitchParameter | ☐ | Do not use context in the query |
| `-WithBeamSearchSamplingStrategy` | SwitchParameter | ☐ | Use beam search sampling strategy |
| `-OnlyResponses` | SwitchParameter | ☐ | Return only responses from the model |
| `-MaxToolcallBackLength` | Int32 | ☐ | The `-MaxToolcallBackLength` parameter. |
