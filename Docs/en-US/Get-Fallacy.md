...
> **SubModule:** GenXdev.AI.Queries | **Type:** Function | **Aliases:** `dispicetext`
...
|:---|:---|:---:|:---|
| `-InputObject` | Object | ✅ | Text to parse to find Fallacies in |
...
| `-AudioTemperature` | Double | ☐ | Controls the temperature for audio<br>generation. |
| `-TemperatureResponse` | Double | ☐ | Controls the temperature for response<br>generation. |
| `-CpuThreads` | Int32 | ☐ | Number of CPU threads to use for processing. |
...
| `-SilenceThreshold` | Double | ☐ | Threshold for detecting silence in audio. |
| `-LengthPenalty` | Double | ☐ | Penalty applied to longer responses. |
| `-EntropyThreshold` | Double | ☐ | Threshold for entropy in response generation. |
| `-LogProbThreshold` | Double | ☐ | Log probability threshold for output. |
| `-NoSpeechThreshold` | Double | ☐ | Threshold for detecting no speech in audio. |
| `-DontSpeak` | SwitchParameter | ☐ | Prevents speaking the response. |
| `-DontSpeakThoughts` | SwitchParameter | ☐ | Prevents speaking the model\''s thoughts. |
| `-NoVOX` | SwitchParameter | ☐ | Disables VOX (voice activation). |
| `-UseDesktopAudioCapture` | SwitchParameter | ☐ | Use desktop audio capture for input. |
| `-NoContext` | SwitchParameter | ☐ | Disables context for the request. |
| `-WithBeamSearchSamplingStrategy` | SwitchParameter | ☐ | Use beam search sampling strategy. |
| `-OnlyResponses` | SwitchParameter | ☐ | Only return responses, no additional data. |
...
> Controls the temperature for audio generation.
...
> Controls the temperature for response generation.
...
> Threshold for detecting silence in audio.
...
> Penalty applied to longer responses.
...
> Threshold for entropy in response generation.
...
> Log probability threshold for output.
...
> Threshold for detecting no speech in audio.
...
> Prevents speaking the response.
...
> Prevents speaking the model\''s thoughts.
...
> Disables context for the request.
...
> Only return responses, no additional data.
