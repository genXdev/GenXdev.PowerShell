...
> **SubModule:** GenXdev.Windows | **Type:** Function | **Aliases:** `sendkeys`, `invokekeys`
...
|:---|:---|:---:|:---|
| `-KeysToSend` | String[] | ✅ | The keyboard input to send as an array of<br>strings |
| `-ProcessName` | String | ☐ | Name of the process to send keys to (supports<br>wildcards) 🌐 wildcards |
| `-ProcessId` | Int32 | ☐ | ID of the process to send keys to |
| `-WindowHandle` | Int64 | ☐ | Window handle to send keys to |
| `-SendKeyEscape` | SwitchParameter | ☐ | Escape control characters and modifiers in<br>input text |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | ☐ | Hold keyboard focus on target window after<br>sending keys |
| `-SendKeyUseShiftEnter` | SwitchParameter | ☐ | Use Shift+Enter instead of Enter for newlines |
