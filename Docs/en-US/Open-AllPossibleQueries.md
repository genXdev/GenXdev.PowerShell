...
> **SubModule:** GenXdev.Queries | **Type:** Function | **Aliases:** `qq`
...
| `-ShowWindow` | SwitchParameter | ☐ | Opens in fullscreen mode |
...
| `-SessionOnly` | SwitchParameter | ☐ | Use session-only mode for browser profile. |
| `-ClearSession` | SwitchParameter | ☐ | Clear browser session before opening. |
| `-SkipSession` | SwitchParameter | ☐ | Skip browser session restore. |
...
### Open-AllPossibleQueries -Queries "powershell scripting" -Monitor 0 Opens all possible query types for "powershell scripting" on the default monitor.
...
### qq "https://github.com" -m -1 Opens all possible query types for the GitHub URL using alias and short parameters.
...
- [ConvertTo-Uris](https://github.com/genXdev/GenXdev.PowerShell/blob/main/Docs/en-US/ConvertTo-Uris.md)
