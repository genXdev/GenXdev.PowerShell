...
> **SubModule:** GenXdev.Queries.Webbrowser | **Type:** Function | **Aliases:** `wikinl`
...
| `-NoBorders` | SwitchParameter | ☐ | Removes window borders from the browser<br>window |
| `-SideBySide` | SwitchParameter | ☐ | Opens two browser windows side by side |
| `-SessionOnly` | SwitchParameter | ☐ | Use session-only cookies (no persistent<br>cookies) |
| `-ClearSession` | SwitchParameter | ☐ | Clear browser session data before opening |
...
> Use session-only cookies (no persistent cookies)
...
> Clear browser session data before opening
