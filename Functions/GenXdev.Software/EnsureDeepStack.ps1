...
function EnsureDeepStack {
    ...
    param(
        ...
        [Parameter(
            Mandatory = $false,
            HelpMessage = ('Automatically consent to DeepStack Docker image ' +
                'download and set persistent flag.')
        )]
        [switch] $AutoConsent,
    ...
    process {
        ...
        try {
            ...
            GenXdev\EnsureDeepStack @params
"@)) -JobDescription 'Install DeepStack Face Recognition Service';

                return;
            }

            # confirm installation consent before pulling Docker image
            $deepStackConsent = GenXdev\Confirm-InstallationConsent `
                -ApplicationName 'DeepStack Face Recognition' `
                -Source 'Docker Hub (deepquestai/deepstack)' `
                -Description ('Face detection, registration, and recognition ' +
                'REST API service running in a Docker container') `
                -Publisher 'DeepQuest AI' `
                -AutoConsent:$AutoConsent `
                -AutoConsentAllPackages:$AutoConsentAllPackages

            if (-not $deepStackConsent) {
                throw 'Installation consent denied for DeepStack Docker image'
            }
