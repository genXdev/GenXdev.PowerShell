...
function Expand-Path {
    ...
    process {
        ...
        if ($normalizedPath.StartsWith([IO.Path]::DirectorySeparatorChar + [IO.Path]::DirectorySeparatorChar)) {
            ...
            try {
                # Use ProviderPath for the raw filesystem path (e.g.
                # \\server\share instead of the provider-qualified
                # Microsoft.PowerShell.Core\FileSystem::\\server\share).
                # $pwd returns the provider-qualified string that
                # System.IO.Path.Combine cannot parse as a rooted path.
                $currentLocation = (Microsoft.PowerShell.Management\Get-Location).ProviderPath
                $normalizedPath = [System.IO.Path]::GetFullPath(
                    [System.IO.Path]::Combine($currentLocation, $normalizedPath))
            }
            catch {
