...
function Set-FoundLocation {
    ...
    begin {
        ...
        # Exclude the current working directory from Find-Item results.
        # If $PWD matches the search criteria, Find-Item would find it
        # and the end-block Where-Object filter discards it anyway —
        # but by then we've wasted time searching the tree.  Adding it
        # as a literal Exclude pattern saves that wasted work.
        $invocationParams.Exclude = @($invocationParams.Exclude) +
