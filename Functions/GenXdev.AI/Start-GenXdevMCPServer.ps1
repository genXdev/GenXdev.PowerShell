function Start-GenXdevMCPServer {
    ...
    function HandleGenXdevMCPServerMCPRequest {
        ...
        try {
            ...
            # Consume injected keys so they don't exit the server loop
            # (the outer while loop checks -not [Console]::KeyAvailable)
        }
    }
    ...
    if ($ExposedCmdLets.Count -eq 0) {
        $ExposedCmdLets = @(
            ...
            [GenXdev.Helpers.ExposedCmdletDefinition]@{
                ...
                AllowedParams                        = @(
                    'OnlyCompleted', 'ExcludeCompleted', 'First'
                )
                DontShowDuringConfirmationParamNames = @()
                ForcedParams                         = @()
                JsonDepth                            = 2
    ...
    if ($PSCmdlet.ShouldProcess("MCP Server on port $Port", "Start server")) {
        ...
        # Main server loop – infinite, auto-restarts on recoverable errors.
        # Only Ctrl+C (PipelineStoppedException) can stop it.
        while ($true) {
            try {
                # (Re)start if the listener was stopped by a previous
                # error.  Use Stop() (not Close()) so the instance is
                # reusable; Close() is final-shutdown only.
                if (-not $listener.IsListening) {
                    try {
                        $listener.Start()
                        Microsoft.PowerShell.Utility\Write-Host 'Server (re)started successfully.' -ForegroundColor Green
                    }
                    catch {
                ...
                # Ctrl+C pressed — clean shutdown, no restart
                ...
                if ($_.Exception.ErrorCode -eq 995) {
                    # WSA_OPERATION_ABORTED – listener was stopped
                    Microsoft.PowerShell.Utility\Write-Verbose 'Listener stopped, restarting...'
                }
                else {
                    Microsoft.PowerShell.Utility\Write-Warning "Server listener error (restarting): $($_.Exception.Message)"
                }
                try { $listener.Stop() } catch { }
                Microsoft.PowerShell.Utility\Start-Sleep 1
            }
            catch {
                Microsoft.PowerShell.Utility\Write-Warning "Unexpected server error (restarting): $($_.Exception.Message)"
                try { $listener.Stop() } catch { }
                Microsoft.PowerShell.Utility\Start-Sleep 1
            }
        }
