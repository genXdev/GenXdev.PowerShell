...
For OneDrive, the path is read from the environment variable set by the
OneDrive sync client, with a registry and default-location fallback.
...
.EXAMPLE
folder OneDrive
Returns the OneDrive folder path from the env var, registry, or default
location
#>
...
'@
    ...
    process {
        ...
        # OneDrive is not a standard SHGetKnownFolderPath folder;
        # resolve it via env var, registry, or default location
        if ($KnownFolder -eq 'OneDrive') {

            if ([PSGenXdevCmdlet]::IsRunningUnderPester()) {

                return (GenXdev\Expand-Path "$($ENV:TEMP)\GenXdev.PowerShell\OneDrive\" -CreateDirectory)
            }

            # 1) Try the environment variable set by the OneDrive client
            $oneDrivePath = [System.Environment]::GetEnvironmentVariable(
                'OneDrive',
                [System.EnvironmentVariableTarget]::User
            )

            if ($oneDrivePath -and (Microsoft.PowerShell.Management\Test-Path -LiteralPath $oneDrivePath)) {
                return $oneDrivePath
            }

            # 2) Try registry: HKCU\Software\Microsoft\OneDrive\Accounts\Personal\UserFolder
            try {
                $regPath = Microsoft.PowerShell.Management\Get-ItemProperty `
                    -LiteralPath 'HKCU:\Software\Microsoft\OneDrive\Accounts\Personal' `
                    -Name 'UserFolder' `
                    -ErrorAction SilentlyContinue

                if ($regPath -and $regPath.UserFolder -and (Microsoft.PowerShell.Management\Test-Path -LiteralPath $regPath.UserFolder)) {
                    return $regPath.UserFolder
                }
            }
            catch { }

            # 3) Fallback to default location
            $defaultPath = GenXdev\Expand-Path '~\OneDrive'
            if (Microsoft.PowerShell.Management\Test-Path -LiteralPath $defaultPath) {
                return $defaultPath
            }

            Microsoft.PowerShell.Utility\Write-Warning (
                'OneDrive folder not found. Returning default path.'
            )
            return $defaultPath
        }
