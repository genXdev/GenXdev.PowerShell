Pester\Describe 'Remove-ItemWithFallback' {
    ...
    Pester\BeforeAll {
        ...
        $script:testFile = GenXdev\Expand-Path "${script:testRoot}\fallback-test.txt" -CreateFile
    ...
    Pester\AfterAll {
        ...
        if (![string]::IsNullOrEmpty($script:testRoot) -and (Microsoft.PowerShell.Management\Test-Path -LiteralPath $script:testRoot)) {
            ...
            if ($script:testRoot -like "${env:TEMP}*") {
                GenXdev\Remove-AllItems -Confirm:$False  $script:testRoot -DeleteFolder -ErrorAction SilentlyContinue
            }
        }
    }
    ...
    Pester\It 'Removes file using direct deletion' {
        ...
        { GenXdev\Remove-ItemWithFallback -Path $script:testFile -ErrorAction Stop } |
