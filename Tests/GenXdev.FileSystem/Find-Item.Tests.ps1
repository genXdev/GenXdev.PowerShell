Pester\Describe 'Find-Item 1' {
    ...
    Pester\BeforeAll {
        $testRoot = GenXdev\Expand-Path ([System.IO.Path]::GetTempPath() + "\$([DateTime]::UtcNow.Ticks.ToString())\") -CreateDirectory
        $testDir = Microsoft.PowerShell.Management\Join-Path $testRoot ([DateTime]::UtcNow.Ticks.ToString()) 'Find-Item-tests'
        $testDir = GenXdev\Expand-Path "$testDir\tests\" -CreateDirectory
        ...
        $encodingTestDir = GenXdev\Expand-Path "$testRoot\encoding-tests\" -CreateDirectory
        ...
        $tempDriveDir = GenXdev\Expand-Path "$testRoot\TempDriveTest\" -CreateDirectory
    ...
    Pester\AfterAll {
        # Navigate out of any test directory before cleanup
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $env:TEMP -ErrorAction SilentlyContinue

        # Cleanup test folder using the actual $testRoot from BeforeAll
        GenXdev\Remove-AllItems -Confirm:$False $testRoot -DeleteFolder
    ...
    Pester\BeforeEach {
        ...
        Microsoft.PowerShell.Management\Set-Location -LiteralPath (GenXdev\Expand-Path "$testDir\" -CreateDirectory) -ErrorAction SilentlyContinue
    }

    Pester\AfterEach {
        # Navigate out of the test directory before attempting to delete it
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $env:TEMP -ErrorAction SilentlyContinue

        GenXdev\Remove-AllItems -Confirm:$False  $testDir
    }

    # # New tests for non-FileSystem
    # Pester\It 'Finds registry keys' {
    #     $keys = GenXdev\Find-Item 'HKLM:\SOFTWARE\Microsoft*' -Directory
    #     $keys.Count | Pester\Should -GT 0
    # }

    # Pester\It 'Finds certificates by pattern' {
    #     $certs = GenXdev\Find-Item 'Cert:\CurrentUser\My\*' -Content 'CN=*'  -Quiet
    #     $certs.Count | Pester\Should -GT 0
    # }

    # Pester\It 'Finds environment variables' {
    #     $vars = GenXdev\Find-Item 'Env:\P*'
    #     $vars | Pester\Should -Contain 'PATH'
    # }

    Pester\It 'Should work with wildcard in the holding directory' {

        $pattern = GenXdev\Expand-Path "$PSScriptRoot\..\..\..\..\Genx*\*.*.*\functions\genxdev.File*\*.ps1"
        $found = @(GenXdev\Find-Item -SearchMask $pattern)
    ...
    Pester\It 'Should find some files in the root of \' {
        ...
        $pattern = "\"
        $found = @(GenXdev\Find-Item -SearchMask $pattern -NoRecurse)
    ...
    Pester\It 'Finds files by extension' {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\Find-Item-test\" -CreateDirectory
        ...
        $files = GenXdev\Find-Item -SearchMask './*.txt' -PassThru
    ...
    Pester\It 'Finds files by extension seperated by ;' {
        ...
        $files = GenXdev\Find-Item -SearchMask "$testRoot\Find-Item-test\file1.txt;$testRoot\Find-Item-test\file2.txt" -PassThru
    ...
    Pester\It 'Finds files by content pattern' {
        ...
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        Microsoft.PowerShell.Management\New-Item -ItemType Directory -Path 'dir1', 'dir2/subdir' -Force -ErrorAction SilentlyContinue
        'test1' | Microsoft.PowerShell.Utility\Out-File (GenXdev\Expand-Path "$testDir/dir1/file1.txt" -CreateDirectory) -Force
        'test2' | Microsoft.PowerShell.Utility\Out-File (GenXdev\Expand-Path "$testDir/dir2/file2.txt" -CreateDirectory) -Force
        'test3' | Microsoft.PowerShell.Utility\Out-File (GenXdev\Expand-Path "$testDir/dir2/subdir/file3.txt" -CreateDirectory) -Force
        $files = @(GenXdev\Find-Item -Content 'test2' -PassThru -Quiet)
    ...
    Pester\It 'Finds only directories' {
        ...
        'test2' | Microsoft.PowerShell.Utility\Out-File (GenXdev\Expand-Path "$testDir/dir2/file2.txt" -CreateDirectory) -Force
        'test3' | Microsoft.PowerShell.Utility\Out-File (GenXdev\Expand-Path "$testDir/dir2/subdir/file3.txt" -CreateDirectory) -Force
        $dirs = @(GenXdev\Find-Item -Directory -PassThru)
    ...
    Pester\It 'Handles wildcards correctly' {
        ...
        Microsoft.PowerShell.Utility\Write-Verbose "GenXdev\Find-Item '$PSScriptRoot\..\..\..\..\..\mod*es\genX*'  -dir -NoRecurse -PassThru"
        $results = GenXdev\Find-Item "$PSScriptRoot\..\..\..\..\..\mod*es\genX*" -dir -NoRecurse -PassThru | Microsoft.PowerShell.Core\ForEach-Object FullName

        $results | Pester\Should -Contain (GenXdev\Expand-Path "$PSScriptRoot\..\..\..\..\\GenXdev")
    }
    ...
    Pester\It 'Finds files by content pattern' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "$testDir\*.txt" -Content 'test content' -PassThru -Quiet)
    ...
    Pester\It 'Finds only directories when specified 1' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "$testDir" -Directory -PassThru | Microsoft.PowerShell.Utility\Select-Object -ExpandProperty FullName)
    ...
    Pester\It "Should work with pattern `$testDir\subdir*\a*\boom\correctly -Directory" {
        ...
        $null = GenXdev\Expand-Path .\subdir\aap\boom\vuur1\ -CreateDirectory
        $null = GenXdev\Expand-Path .\subdir2\aap\boom\vuur2\ -CreateDirectory
        $null = GenXdev\Expand-Path .\subdir3\vis\vuur3\boom\ -CreateDirectory
        $null = GenXdev\Expand-Path .\subdir\arend\boom\vuur4\ -CreateDirectory
        $null = GenXdev\Expand-Path .\subdir\aap\test\vuur5\ -CreateDirectory
        $null = GenXdev\Expand-Path .\subdir\aap\boom\vuur6\ -CreateDirectory
        $null = GenXdev\Expand-Path .\other\something\here\and\there\aap\boom\ -CreateDirectory

        $found = @(GenXdev\Find-Item -SearchMask "$testDir\subdir*\a*\boom\" -Directory -PassThru | Microsoft.PowerShell.Utility\Select-Object -ExpandProperty FullName)
    ...
    Pester\It "Should work with pattern `"`$testDir\**\boom\`" -Directory -PassThru" {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "$testDir\**\boom\" -Directory -PassThru | Microsoft.PowerShell.Utility\Select-Object -ExpandProperty FullName)
        $found | Pester\Should -Contain "$testDir\subdir\aap\boom\vuur1"
        $found | Pester\Should -Contain "$testDir\subdir2\aap\boom\vuur2"
        $found | Pester\Should -Not -Contain "$testDir\subdir3\vis\vuur3\boom"
        $found | Pester\Should -Contain "$testDir\subdir\arend\boom\vuur4"
    ...
    Pester\It "Should work with pattern: `"`$testRoot\**\aap\boom`" -Directory -PassThru" {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "$testRoot\**\aap\boom" -Directory -PassThru | Microsoft.PowerShell.Utility\Select-Object -ExpandProperty FullName)
        $found | Pester\Should -Contain "$testDir\subdir\aap\boom"
        $found | Pester\Should -Contain "$testDir\subdir2\aap\boom"
        $found | Pester\Should -Contain "$testDir\other\something\here\and\there\aap\boom"
    ...
    Pester\It "Should work with pattern: `"`$testRoot\**\aap\boom\`" -PassThru" {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "$testRoot\**\aap\boom\" -PassThru | Microsoft.PowerShell.Utility\Select-Object -ExpandProperty FullName)
    ...
    Pester\It "Should work with pattern: `"`$testRoot\**\aap\boom\fi*A*.txt`"  -PassThru" {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "$testRoot\**\aap\boom\fi*A*.txt" -PassThru | Microsoft.PowerShell.Utility\Select-Object -ExpandProperty FullName)
    ...
    Pester\It "Should work with pattern: `"`$testRoot\**\aap\boom\fi*B*.txt`" -PassThru" {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "$testRoot\**\aap\boom\fi*B*.txt" -PassThru | Microsoft.PowerShell.Utility\Select-Object -ExpandProperty FullName)
    ...
    Pester\It 'Should find files with certain symbols in the filename' {
        ...
        $null = GenXdev\Expand-Path $path -CreateDirectory -CreateFile
        $found = @(GenXdev\Find-Item -SearchMask "$testDir\*" -PassThru | Microsoft.PowerShell.Utility\Select-Object -ExpandProperty FullName)
    ...
    Pester\It 'Should only show ADS when IncludeAlternateFileStreams is specified' {
        ...
        $found1 = @(GenXdev\Find-Item -SearchMask $testFile)
        ...
        $found2 = @(GenXdev\Find-Item -SearchMask $testFile -IncludeAlternateFileStreams)
    ...
    Pester\It 'Should find specific named streams when using streammask' {
        ...
        $found1 = @(GenXdev\Find-Item -SearchMask "${testFile}:stream1")
        $found1.Count | Pester\Should -Be 1
        ...
        $found2 = @(GenXdev\Find-Item -SearchMask "${testFile}:stream*")
        $found2.Count | Pester\Should -Be 2
        ...
        $found3 = @(GenXdev\Find-Item -SearchMask "${testFile}:")
    ...
    Pester\It 'Should filter streams with pattern matching' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "${testFile}:*" -SearchADSContent -Content 'password\d+' -Quiet)
        $found.Count | Pester\Should -Be 1
        $found[0] | Pester\Should -Be '.\pattern-stream.txt:secret'

        # Verify no match returns empty
        $noMatch = @(GenXdev\Find-Item -SearchMask "${testFile}:*" -SearchADSContent -Content 'nonexistent' -Quiet)
        $noMatch.Count | Pester\Should -Be 0

        # Verify no match returns normal files
        $notMatch = @(GenXdev\Find-Item -SearchMask "${testFile}" -Content 'nonexistent' -Quiet -NotMatch)
        $notMatch.Count | Pester\Should -BeGreaterThan 0

        # Verify no match returns ads files
        $notMatch = @(GenXdev\Find-Item -SearchMask "${testFile}:*" -SearchADSContent -Content 'nonexistent' -Quiet -NotMatch)
        $notMatch.Count | Pester\Should -BeGreaterThan 0
    }
    ...
    Pester\It 'Finds files by name pattern' {
        ...
        $files = GenXdev\Find-Item -SearchMask './file*.txt' -PassThru
        $files.Count | Pester\Should -Be 3
        $files.Name | Pester\Should -Contain 'file1.txt'
        $files.Name | Pester\Should -Contain 'file2.txt'
    ...
    Pester\It 'Should work with wildcard in the holding directory' {
        ...
        $pattern = GenXdev\Expand-Path "$PSScriptRoot\..\..\..\..\Genx*\*.*.*\functions\genxdev.*\*.ps1"
        $found = @(GenXdev\Find-Item -SearchMask $pattern)
    ...
    Pester\It 'Should work with wildcard file paths and combined with stream masks' {
        ...
        $found = GenXdev\Find-Item "$PSScriptRoot\..\..\..\..\..\**\*Docs*\*.md"

        $found.Count | Pester\Should -Not -Be 0
        $found.Count | Pester\Should -BeGreaterThan 30
    }
    ...
    Pester\It 'Should handle wildcards in file and stream patterns' {
        ...
        $found1 = @(GenXdev\Find-Item -SearchMask "$testDir\wildcard*.dat:data")
        ...
        $found2 = @(GenXdev\Find-Item -SearchMask "$testDir\wildcard1*:m*")
    ...
    Pester\It 'Should correctly handle -ads flag vs explicit stream masks' {
        ...
        $found1 = @(GenXdev\Find-Item -SearchMask $testFile -IncludeAlternateFileStreams)
        ...
        $found2 = @(GenXdev\Find-Item -SearchMask "${testFile}:test*")
        $found2.Count | Pester\Should -Be 2
    ...
    Pester\It 'Should work with wildcard file paths and combined with stream masks' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask "$subDir\*.jpg:description.json")
        $found.Count | Pester\Should -Be 2
        ...
        $foundWithPattern = @(
            GenXdev\Find-Item -SearchMask "$subDir\file*.jpg:metadata" -Content 'secret' -SearchADSContent -NoLinks -Quiet
        )
    ...
    Pester\It 'Finds files using -DriveLetter with -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' -DriveLetter $driveLetter -NoRecurse -PassThru
    ...
    Pester\It 'Finds files using -Root with -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' -Root $testDir -NoRecurse -PassThru
        $files.Count | Pester\Should -BeGreaterThan 0
        $files.Name | Pester\Should -Contain 'file1.txt'
    }
    ...
    Pester\It 'Finds files using -SearchDrives with -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' -SearchDrives $testDir -NoRecurse -PassThru
        $files.Count | Pester\Should -BeGreaterThan 0
        $files.Name | Pester\Should -Contain 'file1.txt'
    }

    Pester\It 'Finds files on temporary subst drive with -DriveLetter and -NoRecurse' {
        $files = GenXdev\Find-Item -Name '*.txt' -DriveLetter $freeLetter -NoRecurse -PassThru
        $files.Count | Pester\Should -BeGreaterThan 0
        $files.Name | Pester\Should -Contain 'tempFile.txt'
    }
    ...
    Pester\It 'Finds files on temporary subst drive with -SearchDrives and -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' -SearchDrives $tempDrivePath -NoRecurse -PassThru
        $files.Count | Pester\Should -BeGreaterThan 0
        $files.Name | Pester\Should -Contain 'tempFile.txt'
    }
    ...
    Pester\It 'Finds files on temporary subst drive with -Root and -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' -Root $tempDrivePath -NoRecurse -PassThru
        $files.Count | Pester\Should -BeGreaterThan 0
        $files.Name | Pester\Should -Contain 'tempFile.txt'
    }
    ...
    Pester\It 'Finds files combining -DriveLetter (multiple, including temp) with -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' -DriveLetter $mainDriveLetter, $freeLetter -NoRecurse -PassThru
        $files.Count | Pester\Should -BeGreaterThan 0
    ...
    Pester\It 'Finds files combining -SearchDrives (multiple, including temp) with -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' -SearchDrives $mainDrive, "$($freeLetter):\" -NoRecurse -PassThru
        $files.Count | Pester\Should -BeGreaterThan 0
    ...
    Pester\It 'Finds files combining -Root (multiple, including temp) with -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' -Root $testDir, "$($freeLetter):\" -NoRecurse -PassThru
        $files.Count | Pester\Should -BeGreaterThan 0
        $files.Name | Pester\Should -Contain 'file1.txt'
        $files.Name | Pester\Should -Contain 'tempFile.txt'
    }
    ...
    Pester\It 'Finds files combining all parameters (-DriveLetter, -SearchDrives, -Root) including temp drive with -NoRecurse' {
        ...
        $files = GenXdev\Find-Item -Name '*.txt' `
    ...
    Pester\It 'LimitToRoot prevents directory traversal attacks with ..\..\' {
        ...
        $restrictedRoot = GenXdev\Expand-Path "$testRoot\restricted-zone\" -CreateDirectory
        $outsideRoot = GenXdev\Expand-Path "$testRoot\outside-zone\" -CreateDirectory
        ...
        $result1 = @(GenXdev\Find-Item -Name '..\..\outside-zone\secret.txt' -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        ...
        $result2 = @(GenXdev\Find-Item -Name $absolutePath -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        $result2.Count | Pester\Should -Be 0 -Because "Absolute paths stripped to filename only"

        # Test 3: Attempt with multiple ..\ patterns
        $result3 = @(GenXdev\Find-Item -Name '..\..\..\..\..\outside-zone\secret.txt' -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        ...
        $result4 = @(GenXdev\Find-Item -Name 'subdir\..\..\outside-zone\secret.txt' -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        ...
        $result5 = @(GenXdev\Find-Item -Name '\\server\share\secret.txt' -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        $result5.Count | Pester\Should -Be 0 -Because "UNC paths stripped to filename only"

        # Test 6: Forward slash variant
        $result6 = @(GenXdev\Find-Item -Name '../../outside-zone/secret.txt' -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        ...
        $result7 = @(GenXdev\Find-Item -Name 'allowed.txt' -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        ...
        $result8 = @(GenXdev\Find-Item -Name 'subdir\somefile.txt' -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        ...
        $result9 = @(GenXdev\Find-Item -Name '*.txt' -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
        ...
        $result10 = @(GenXdev\Find-Item -Name $drivePath -Root $restrictedRoot -LimitToRoot -PassThru -NoRecurse)
    ...
    Pester\It 'LimitToRoot with multiple Root directories enforces restriction for each' {
        ...
        $zone1 = GenXdev\Expand-Path "$testRoot\zone1\" -CreateDirectory
        $zone2 = GenXdev\Expand-Path "$testRoot\zone2\" -CreateDirectory
        $forbidden = GenXdev\Expand-Path "$testRoot\forbidden\" -CreateDirectory
        ...
        $result = @(GenXdev\Find-Item -Name '..\..\forbidden\secret.txt' -Root $zone1, $zone2 -LimitToRoot -PassThru -NoRecurse)
        ...
        $result2 = @(GenXdev\Find-Item -Name '*.txt' -Root $zone1, $zone2 -LimitToRoot -PassThru -NoRecurse)
    ...
    Pester\It 'LimitToRoot without Root parameter should still strip directory components' {
        # Setup
        $testDir = GenXdev\Expand-Path "$testRoot\limitroot-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        'current dir file' | Microsoft.PowerShell.Utility\Out-File "$testDir\current.txt" -Force

        $parentDir = GenXdev\Expand-Path "$testRoot\"
        ...
        $result = @(GenXdev\Find-Item -Name '..\parent.txt' -LimitToRoot -PassThru -NoRecurse)
        ...
        $result2 = @(GenXdev\Find-Item -Name 'current.txt' -LimitToRoot -PassThru -NoRecurse)
    ...
    Pester\It 'Finds files with Context parameter - pre and post context' {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\Find-Item-context-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'target' -Context 2, 3 -NoRecurse)
    ...
    Pester\It 'Finds files with Context parameter - pre-context only' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'keyword' -Context 2, 0 -NoRecurse)
        $found.Count | Pester\Should -BeGreaterThan 0
    ...
    Pester\It 'Finds files with Context parameter - post-context only' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'findme' -Context 0, 3 -NoRecurse)
        $found.Count | Pester\Should -BeGreaterThan 0
    ...
    Pester\It 'Finds files with Context parameter - single value applies to both' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'pattern' -Context 1 -NoRecurse)
        $found.Count | Pester\Should -BeGreaterThan 0
    ...
    Pester\It 'Finds files with Context parameter works without breaking normal functionality' {
        ...
        $foundNormal = @(GenXdev\Find-Item -SearchMask $testFile -Content 'special' -NoRecurse)
        $foundNormal.Count | Pester\Should -BeGreaterThan 0

        # Test with Context parameter - should also work
        $foundWithContext = @(GenXdev\Find-Item -SearchMask $testFile -Content 'special' -Context 1, 1 -NoRecurse)
    ...
    Pester\It 'Finds files with Context parameter handles edge cases' {
        ...
        $foundBeginning = @(GenXdev\Find-Item -SearchMask $testFile1 -Content 'target' -Context 2, 2 -NoRecurse)
        $foundBeginning.Count | Pester\Should -BeGreaterThan 0

        # Test match at end with context [2,2]
        $foundEnd = @(GenXdev\Find-Item -SearchMask $testFile2 -Content 'target' -Context 2, 2 -NoRecurse)
    ...
    Pester\It 'Handles UTF-32 Big Endian with multi-byte characters correctly' {
        ...
        $foundBaseline = @(GenXdev\Find-Item -SearchMask $testFile -Content 'world' -Encoding 'UTF8' -PassThru -Quiet)
        ...
        try {
            ...
            $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'world' -Encoding 'BigEndianUTF32' -PassThru -Quiet)
            ...
            Microsoft.PowerShell.Utility\Write-Verbose "BigEndianUTF32 encoding not fully supported: $($_.Exception.Message)"
        }

        # Test with wrong encoding - should gracefully handle (ASCII can't read UTF-32)
        $notFound = @(GenXdev\Find-Item -SearchMask $testFile -Content 'world' -Encoding 'ASCII' -PassThru -Quiet)
    ...
    Pester\It 'Handles Big5 Chinese Traditional encoding correctly' {
        ...
        try {
            ...
            $found = @(GenXdev\Find-Item -SearchMask $testFile -Content '繁體中文' -Encoding '950' -PassThru -Quiet)
            $found.Count | Pester\Should -Be 1

            # Test with wrong encoding
            $notFound = @(GenXdev\Find-Item -SearchMask $testFile -Content '繁體中文' -Encoding 'UTF8' -PassThru -Quiet)
            $notFound.Count | Pester\Should -Be 0
        }
        catch {
    ...
    Pester\It 'Handles KOI8-R Cyrillic encoding correctly' {
        ...
        try {
            ...
            $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'Русский' -Encoding 'Default' -PassThru -Quiet)
            $found.Count | Pester\Should -BeGreaterOrEqual 0  # May not work on all systems

            # Test graceful handling with ASCII
            $result = @(GenXdev\Find-Item -SearchMask $testFile -Content 'КОИ8' -Encoding 'ASCII' -PassThru -Quiet)
    ...
    Pester\It 'Handles Windows-1251 Cyrillic encoding correctly' {
        ...
        try {
            ...
            $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'Привет' -Encoding '1251' -PassThru -Quiet)
            $found.Count | Pester\Should -Be 1

            # Test with string name
            $found2 = @(GenXdev\Find-Item -SearchMask $testFile -Content 'мир' -Encoding 'Default' -PassThru -Quiet)
    ...
    Pester\It 'Handles UTF-8 BOM vs No-BOM correctly' {
        ...
        $foundBOM = @(GenXdev\Find-Item -SearchMask $testFileBOM -Content 'émojis' -Encoding 'UTF8BOM' -PassThru -Quiet)
        $foundBOM.Count | Pester\Should -Be 1

        # Both should be found with UTF8NoBOM encoding
        $foundNoBOM = @(GenXdev\Find-Item -SearchMask $testFileNoBOM -Content 'ñiño' -Encoding 'UTF8NoBOM' -PassThru -Quiet)
        $foundNoBOM.Count | Pester\Should -Be 1

        # Both should be found with generic UTF8 encoding
        $foundGeneric1 = @(GenXdev\Find-Item -SearchMask $testFileBOM -Content '🎉' -Encoding 'UTF8' -PassThru -Quiet)
        $foundGeneric1.Count | Pester\Should -Be 1

        $foundGeneric2 = @(GenXdev\Find-Item -SearchMask $testFileNoBOM -Content '🎉' -Encoding 'UTF8' -PassThru -Quiet)
    ...
    Pester\It 'Handles extreme Unicode ranges and surrogate pairs' {
        ...
        $foundEmoji = @(GenXdev\Find-Item -SearchMask $testFile -Content '🎭' -Encoding 'UTF8' -PassThru -Quiet)
        $foundEmoji.Count | Pester\Should -Be 1

        # Test finding mathematical symbols
        $foundMath = @(GenXdev\Find-Item -SearchMask $testFile -Content '∀∃' -Encoding 'UTF8' -PassThru -Quiet)
        $foundMath.Count | Pester\Should -Be 1

        # Test finding CJK characters
        $foundCJK = @(GenXdev\Find-Item -SearchMask $testFile -Content '你好世界' -Encoding 'UTF8' -PassThru -Quiet)
        ...
        $wrongEncoding = @(GenXdev\Find-Item -SearchMask $testFile -Content 'Basic.*Hello' -Encoding 'ASCII' -PassThru -Quiet)
    ...
    Pester\It 'Validates buffer sizing with worst-case encoding expansions' {
        ...
        try {
            ...
            $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'AAAAAAAAAA' -Encoding 'UTF32' -PassThru -Quiet)
            if ($found.Count -eq 0) {
                ...
                $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'AAAA' -Encoding 'UTF32' -PassThru -Quiet)
            }
            ...
            $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'AAAAAAAAAA' -Encoding 'UTF8' -PassThru -Quiet)
            $found.Count | Pester\Should -Be 1
        }
        ...
        $foundUTF8 = @(GenXdev\Find-Item -SearchMask $testFile -Content '🌍🌍🌍🌍🌍' -Encoding 'UTF8' -PassThru -Quiet)
    ...
    Pester\It 'Handles mixed encoding content gracefully' {
        ...
        try {
            ...
            $foundASCII = @(GenXdev\Find-Item -SearchMask $testFile -Content 'ASCII text' -Encoding 'UTF8' -PassThru -Quiet)
            ...
            $foundCorrect = @(GenXdev\Find-Item -SearchMask $testFile -Content 'ASCII text' -Encoding 'Default' -PassThru -Quiet)
    ...
    Pester\It 'Tests ANSI encoding with current culture' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'special chars' -Encoding 'ANSI' -PassThru -Quiet)
        $found.Count | Pester\Should -Be 1

        # Test with Default parameter
        $foundDefault = @(GenXdev\Find-Item -SearchMask $testFile -Content 'çñü' -Encoding 'Default' -PassThru -Quiet)
    ...
    Pester\It 'Validates encoding parameter error handling' {
        ...
        $result = @(GenXdev\Find-Item -SearchMask $testFile -Content 'Simple' -Encoding 'UTF8' -PassThru -Quiet)
        ...
        foreach ($encoding in $validEncodings) {
            $testResult = @(GenXdev\Find-Item -SearchMask $testFile -Content 'Simple' -Encoding $encoding -PassThru -Quiet)
    ...
    Pester\It 'Tests encoding with context lines and special characters' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask $testFile -Content '测试' -Encoding 'UTF8' -Context 1, 1 -NoRecurse)
        $found.Count | Pester\Should -BeGreaterThan 0
    ...
    Pester\It 'NotMatch parameter - finds files that do not contain specified pattern' -Skip {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content 'target' -NotMatch -PassThru -Quiet)
        $found.Count | Pester\Should -Be 2
    ...
    Pester\It 'NotMatch parameter - works with regex patterns' -Skip {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-regex-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content '\d+' -NotMatch -PassThru -Quiet)
        $found.Count | Pester\Should -Be 2
    ...
    Pester\It 'NotMatch parameter - works with SimpleMatch' -Skip {
        # Setup test files
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-simple-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content '[pattern]' -NotMatch -SimpleMatch -PassThru -Quiet)
        $found.Count | Pester\Should -Be 2
    ...
    Pester\It 'NotMatch parameter - works with CaseSensitive' -Skip {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-case-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content 'UPPERCASE' -NotMatch -CaseSensitive -PassThru -Quiet)
        $found.Count | Pester\Should -Be 3
    ...
    Pester\It 'NotMatch parameter - works with AllMatches' -Skip {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-allmatch-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content 'target' -NotMatch -AllMatches -PassThru -Quiet)
        $found.Count | Pester\Should -Be 1
    ...
    Pester\It 'NotMatch parameter - works with Context parameter' -Skip {
        # Setup test file
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-context-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content 'target' -NotMatch -Context 1, 1 -NoRecurse)
        $found.Count | Pester\Should -BeGreaterThan 0
    ...
    Pester\It 'NotMatch parameter - works with Culture parameter' -Skip {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-culture-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content 'café' -NotMatch -SimpleMatch -Culture 'fr-FR' -PassThru -Quiet)
        $found.Count | Pester\Should -Be 1
    ...
    Pester\It 'NotMatch parameter - works with different encoding' -Skip {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-encoding-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content '测试' -NotMatch -Encoding 'UTF8' -PassThru -Quiet)
        $found.Count | Pester\Should -Be 2
    ...
    Pester\It 'NotMatch parameter - works in combination with Directory parameter' {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-directory-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*' -Directory -PassThru | Microsoft.PowerShell.Core\Where-Object { $_.Name -notmatch 'target' })
        $found.Count | Pester\Should -BeGreaterThan 0
    ...
    Pester\It 'NotMatch parameter - works with Quiet parameter' -Skip {
        # Setup test files
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-quiet-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -Content 'pattern' -NotMatch -Quiet)
        $found.Count | Pester\Should -Be 1
    ...
    Pester\It 'NotMatch parameter - edge case with empty files' -Skip {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-empty-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
    ...
    Pester\It 'NotMatch parameter - works without Content parameter for filename matching' {
        ...
        $testDir = GenXdev\Expand-Path "$testRoot\NotMatch-filename-test\" -CreateDirectory
        Microsoft.PowerShell.Management\Set-Location -LiteralPath $testDir
        ...
        $found = @(GenXdev\Find-Item -SearchMask '*.txt' -PassThru | Microsoft.PowerShell.Core\Where-Object { $_.Name -notmatch 'target' })
        $found.Count | Pester\Should -Be 2
        $foundNames = $found.Name
    ...
    Pester\It 'Culture parameter - Turkish i/I distinction should be respected with Turkish culture' {
        ...
        $foundTurkish = @(GenXdev\Find-Item -SearchMask $testFile -Content 'istanbul' -SimpleMatch -Culture 'tr-TR' -PassThru -Quiet)
        $foundCurrent = @(GenXdev\Find-Item -SearchMask $testFile -Content 'istanbul' -SimpleMatch -PassThru -Quiet)
        ...
        if ((Microsoft.PowerShell.Utility\Get-Culture).Name -ne 'tr-TR') {
            Microsoft.PowerShell.Utility\Write-Verbose "Turkish culture found: $($foundTurkish.Count), Current culture found: $($foundCurrent.Count)"
    ...
    Pester\It 'Culture parameter - German ß (sharp s) should match SS in German culture' {
        ...
        $foundBeta = @(GenXdev\Find-Item -SearchMask $testFile -Content 'straße' -SimpleMatch -Culture 'de-DE' -PassThru -Quiet)
        $foundSS = @(GenXdev\Find-Item -SearchMask $testFile -Content 'strasse' -SimpleMatch -Culture 'de-DE' -PassThru -Quiet)
    ...
    Pester\It 'Culture parameter - Case-sensitive with culture should still respect culture rules' {
        ...
        $foundCaseSensitive = @(GenXdev\Find-Item -SearchMask $testFile -Content 'istanbul' -SimpleMatch -Culture 'tr-TR' -CaseSensitive -PassThru -Quiet)
        $foundCaseInsensitive = @(GenXdev\Find-Item -SearchMask $testFile -Content 'istanbul' -SimpleMatch -Culture 'tr-TR' -PassThru -Quiet)
    ...
    Pester\It 'Culture parameter - No culture specified should use current culture' {
        ...
        $foundNoCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'café' -SimpleMatch -PassThru -Quiet)
        $foundCurrentCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'café' -SimpleMatch -Culture (Microsoft.PowerShell.Utility\Get-Culture).Name -PassThru -Quiet)
    ...
    Pester\It 'Culture parameter - Invalid culture should not crash' {
        ...
        try {
            $result = @(GenXdev\Find-Item -SearchMask $testFile -Content 'Simple' -SimpleMatch -Culture 'invalid-XX' -PassThru -Quiet)
    ...
    Pester\It 'Culture parameter - French accented characters should match unaccented in French culture' {
        ...
        $foundAccented = @(GenXdev\Find-Item -SearchMask $testFile -Content 'café' -SimpleMatch -Culture 'fr-FR' -PassThru -Quiet)
        $foundUnaccented = @(GenXdev\Find-Item -SearchMask $testFile -Content 'cafe' -SimpleMatch -Culture 'fr-FR' -PassThru -Quiet)
    ...
    Pester\It 'Culture parameter - Ligatures should be handled correctly in specific cultures' {
        ...
        $foundLigature = @(GenXdev\Find-Item -SearchMask $testFile -Content 'aesthetic' -SimpleMatch -Culture 'en-US' -PassThru -Quiet)
        $foundNoLigature = @(GenXdev\Find-Item -SearchMask $testFile -Content 'æsthetic' -SimpleMatch -Culture 'en-US' -PassThru -Quiet)
    ...
    Pester\It 'Culture parameter - Complex regex should not use culture (only SimpleMatch should)' {
        ...
        $foundRegexWithCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'i.*bul' -Culture 'tr-TR' -PassThru -Quiet)
        $foundRegexWithoutCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'i.*bul' -PassThru -Quiet)
        ...
        $foundSimpleWithCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'istanbul' -SimpleMatch -Culture 'tr-TR' -PassThru -Quiet)
        $foundSimpleWithoutCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'istanbul' -SimpleMatch -PassThru -Quiet)
        ...
        if ((Microsoft.PowerShell.Utility\Get-Culture).Name -ne 'tr-TR') {
            ...
            Microsoft.PowerShell.Utility\Write-Verbose "SimpleMatch with Turkish culture found: $($foundSimpleWithCulture.Count), without: $($foundSimpleWithoutCulture.Count)"
        }
    }
    ...
    Pester\It 'Culture parameter - Culture comparison with Context lines should work correctly' {
        ...
        $foundWithContext = @(GenXdev\Find-Item -SearchMask $testFile -Content 'istanbul' -SimpleMatch -Culture 'tr-TR' -Context 1, 2 -NoRecurse)
        ...
        if ($foundWithContext.Count -gt 0) {
            ...
            Microsoft.PowerShell.Utility\Write-Verbose "Context output: $contextOutput"
        }
        $contextOutput = $foundWithContext -join "`n"
    ...
    Pester\It 'Culture parameter - AllMatches with culture should find all culture-equivalent matches' {
        ...
        $foundAllMatches = @(GenXdev\Find-Item -SearchMask $testFile -Content 'cafe' -SimpleMatch -Culture 'fr-FR' -AllMatches -NoRecurse)
        ...
        Microsoft.PowerShell.Utility\Write-Verbose "AllMatches output: $matchOutput"

        # Test that culture is being applied consistently
        $foundSingle = @(GenXdev\Find-Item -SearchMask $testFile -Content 'cafe' -SimpleMatch -Culture 'fr-FR' -NoRecurse)
    ...
    Pester\It 'Culture parameter - Basic functionality test ensures culture parameter is processed' {
        ...
        $foundWithCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'ASCII' -SimpleMatch -Culture 'en-US' -PassThru -Quiet)
        $foundWithoutCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'ASCII' -SimpleMatch -PassThru -Quiet)
        ...
        try {
            $foundInvalidCulture = @(GenXdev\Find-Item -SearchMask $testFile -Content 'ASCII' -SimpleMatch -Culture 'zz-ZZ' -PassThru -Quiet)
    ...
    Pester\It 'Culture parameter - Performance test ensures culture setup is not done repeatedly' {
        ...
        $found = @(GenXdev\Find-Item -SearchMask $testFile -Content 'café' -SimpleMatch -Culture 'fr-FR' -AllMatches -PassThru -Quiet)
        ...
        Microsoft.PowerShell.Utility\Write-Verbose "Culture performance test: Found $($found.Count) matches in $($duration)ms"
    }
}
